...
//float f1000_array[3] = {1.20, 2.40, 3.60};
...
int main() {
	...
	float dot_product_C;
	float32_t dot_product_DSP;
	float dot_product_asm;
	float variance_C;
	float32_t variance_DSP;
	float variance_asm;
	float mean;
	float temp;
	...
	for(i=0; i<VECTOR_SIZE; i++) {
		dot_product_C += f1000_array[i]*f1000_array[i];
	}
	printf("Dot Product C: %f\n", dot_product_C);
	
	//Dot Product CMSIS-DSP Implementation
	arm_dot_prod_f32(f1000_array,f1000_array, VECTOR_SIZE, &dot_product_DSP);
	printf("Dot Product DSP: %f\n", dot_product_DSP);
	
	asm_dp(f1000_array, f1000_array, VECTOR_SIZE, &dot_product_asm);
	printf("Dot Product assembly: %f\n", dot_product_asm);
	
	/**-----------------------------------------------------------------------------------------------------------**/
	
	//The answers of variance for the C and DSP implementation are different becuase the DSP routine
	//divides by (N-1) whereas the formula we are asked to use divides by N.
	
	//Variance C Implementation
	for(i=0; i<VECTOR_SIZE; i++) {
		mean += f1000_array[i];
	}
	mean = mean/VECTOR_SIZE;
	for(i=0; i<VECTOR_SIZE; i++) {
		temp = (f1000_array[i]-mean);
		temp = temp*temp;
		variance_C += temp;
	}
	variance_C = variance_C/(VECTOR_SIZE-1);
	printf("Variance C: %f\n", variance_C);
	
	//Variance DSP Implementation
	arm_var_f32(f1000_array, VECTOR_SIZE, &variance_DSP);
	printf("Variance DSP: %f\n", variance_DSP);
	
	//Variance assembly Implementation
	asm_var(f1000_array, VECTOR_SIZE, &variance_asm);
	printf("Variance Assembly: %f\n", variance_asm);
